...
# Состояния
(BUSY_SELECT_START_DATE, BUSY_SELECT_END_DATE, BUSY_SELECT_START_TIME,
 BUSY_SELECT_END_TIME, BUSY_ENTER_LABEL, BUSY_CONFIRM) = range(6)
...
async def busy_calendar_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cur_step == BUSY_SELECT_END_DATE:
...
async def busy_time_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cur_step == BUSY_SELECT_END_TIME:
...
async def busy_enter_label(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if start_dt >= end_dt:
        await update.message.reply_text(
            "Время конца должно быть позже времени начала. Повторите /busy."
        )
        return ConversationHandler.END
...
async def busy_confirm_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "save_busy_yes":
        ...
        if not p:
            ...
            return ConversationHandler.END
        db.add_busy(query.from_user.id, p["label"], p["start_ts"], p["end_ts"])
...
async def task_enter_duration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        db.add_task(update.effective_user.id, label, dur, None)
...
async def my_busy_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    rows = db.get_busy(update.effective_user.id)
    if not rows:
...
async def my_tasks_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    rows = db.get_tasks(update.effective_user.id)
    if not rows:
...
async def schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    schedule_data = scheduler.build_week_schedule(update.effective_user.id)
    today = datetime.now().date()
    ...
    for i in range(7):
        ...
        items = sorted(schedule_data.get(d, []), key=lambda x: x['start'])
        if not items:
            msg += "  - Свободно\n"
        for it in items:
...
async def set_hours_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Введите рабочие часы в формате: `start end` (например: 8 17)"
    )
...
async def set_hours_handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) != 2:
        await update.message.reply_text(
            "Неправильный формат. Попробуйте снова: два числа, например `8 17`"
        )
        return SET_HOURS_WAIT
    ...
    try:
        ...
        if not (0 <= sh < 24 and 0 < eh <= 24 and sh < eh):
            raise ValueError()
        db.set_user_hours(update.effective_user.id, sh, eh)
